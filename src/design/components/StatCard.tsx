...
export function StatCard({
  ...
  return (
    <Card className={cn('p-5', className)}>
      <span className="h-9.5 w-9.5 rounded-[12px] bg-accent-soft inline-flex items-center justify-center">
        <Icon size={17} className="text-accent-strong" />
      </span>
      <p className={cn('font-display text-[22px] font-bold tnum mt-3 leading-tight', toneClass)}>
        {value}
      </p>
      <p className="text-[11.5px] font-medium text-ink-soft uppercase tracking-[0.06em] mt-1">
        {label}
      </p>
